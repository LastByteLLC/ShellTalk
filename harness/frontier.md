...
|---|---:|---:|---:|---:|---:|---:|---:|---|
| **shipped (main)** | **454** | **0.9626** | **0.9824** | **0.9732** | **0.9155** | **0.870** | 358 | Run 2026-04-24-tier-1-2: Tier 1 (T1.1-T1.8) + Tier 2 (T2.1-T2.4) all shipped. New slot types `.fileSize`/`.relativeDays`/`.commandFlag`; `.host` entity + multi-word proper-noun chunker; templates `git_commit_push`, `swift_build_and_test`, `find_mtime_range`, `git_log_date_range`, `yq_parse`; multi-source extraction; healing flag-correction map; 156 tests. |

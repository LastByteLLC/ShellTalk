...
let allCases: [(String, [EvalCase])] = [
  ("FileOps", [
    ...
    EvalCase("make script.sh executable", template: "chmod_perms", category: "file_ops", required: ["chmod"], "Natural: executable"),
  ...
  ("Network", [
    ...
    EvalCase("dns lookup for google.com", template: "host_lookup", category: "network", required: ["host"], "Natural: dns=host"),
  ...
  ("TerseWithFlags", [
    ...
    EvalCase("chmod +x deploy.sh", template: "chmod_perms", category: "file_ops", required: ["chmod"], "Terse+flags: chmod +x"),
    ...
    for (suite, cases) in allCases {
      ...
      for tc in cases {
        ...
        for s in tc.requiredSubstrings {
          substrTotal += 1
          if r?.command.contains(s) == true { substrOk += 1 }
        }
        ...
        if !tOk {
          failures.append((suite, q, tc.expectedTemplateId, act, r?.categoryId ?? "(nil)", cmd))
        }
      }
